...
__txn_get_snapshot_int(WT_SESSION_IMPL *session, bool publish)
{
    ...
    txn_global = &conn->txn_global;
    txn_shared = WT_SESSION_TXN_SHARED(session);
    // 关注一下 txn[更详细更具体] 和 txn_shared[基本上时间戳相关，每个session也是只有一个] 区别
    // 覆盖掉旧的snapshot数组，复用内存空间，从头开始使用
    n = 0;

    /* Fast path if we already have the current snapshot. */
    // 如果我们已经拥有当前快照，则可以快速得到 snapshot。
     ...
     ** 包括检查点事务(如果正在运行): 我们应该忽略检查点写入元数据的任何未提交的更改。
     ** 我们不必将检查点的更改固定，因此不要将其包含在已发布的固定 ID 中。
     **** 这句好没太理解是啥意思，in the published pinned ID 这个不太清楚干啥用的
     ...
     **  我们可以假设，如果一个函数在无意发布的情况下调用，那么它是检查点调用它两次的特殊情况。
     **  在这种情况下，snapshot 不用包括检查点 ID。
     */
    if ((id = txn_global->checkpoint_txn_shared.id) != WT_TXN_NONE) {
        // checkpoint id 也收集在 snapshot 中
        if (txn->id != id)
            txn->snapshot[n++] = id;
        if (publish)
    ...
    // 对于纯只读操作，current_id 定然分配的有些特殊情况，需要重点关注一下
    if (prev_oldest_id == current_id) {
        pinned_id = current_id;
        /* Check that the oldest ID has not moved in the meantime. */
        // oldest_id 留意一下在什么情况下移动
        WT_ASSERT(session, prev_oldest_id == txn_global->oldest_id);
        goto done;
    }

    /* Walk the array of concurrent transactions. */
    // 遍历并发事务数组，查看所有session的txn
    WT_ORDERED_READ(session_cnt, conn->session_cnt);
    WT_STAT_CONN_INCR(session, txn_walk_sessions);
    for (i = 0, s = txn_global->txn_shared_list; i < session_cnt; i++, s++) {
        WT_STAT_CONN_INCR(session, txn_sessions_walked);
         ...
         **      ID 如果它比我们看到的最旧的 ID 更旧。 如果我们与正在分配 ID 的线程竞争，
         **      就会发生这种情况——该 ID 将不会被使用，因为该线程将继续旋转，直到它获得一个有效的 ID。
         ...
         **      ID 是否高于我们看到的当前 ID。 如果事务已经完成，就会发生这种情况。
         **      在这种情况下，我们忽略此事务，因为它对当前快照不可见。
         */
        while (s != txn_shared && (id = s->id) != WT_TXN_NONE && WT_TXNID_LE(prev_oldest_id, id) &&
          WT_TXNID_LT(id, current_id)) {
            ...
            if (!s->is_allocating) {
                 ...
                 * allocation.
                 **  ID分配后仍有可能获取的ID无效，再次检查。
                 **  事务 ID 的读取要小心有序：我们希望在该事务完成 ID 分配后从事务状态中重新读取 ID。
                 */
                WT_READ_BARRIER();
                if (id == s->id) {
                    txn->snapshot[n++] = id;
     ...
     **  如果我们获得了新快照，请更新此会话的已发布固定 ID。
     ** 不希望在获取过程中 oldest id 出现变化，毕竟读取过程中都上锁了
     */
    WT_ASSERT(session, WT_TXNID_LE(prev_oldest_id, pinned_id));
    WT_ASSERT(session, prev_oldest_id == txn_global->oldest_id);
done:
    // pinned_id 为此时 snaphost 数组中最小 id
    if (publish)
        txn_shared->pinned_id = pinned_id;
    // 释放 reader 锁
    __wt_readunlock(session, &txn_global->rwlock);
    // 此处还有个重要的排序逻辑，同时记录了snapshot大小
...
__wt_txn_update_oldest(WT_SESSION_IMPL *session, uint32_t flags)
{
    ...
    /* Try to move the pinned timestamp forward. 反正没看懂就算了 */
    if (strict)
     ...
     ** 现在重新扫描，我们拥有独占访问权限。 这是必要的，因为线程获取带有读锁的事务快照，
     ** 我们必须确保没有线程在本地获得快照但尚未发布其 snap_min。
     */
    __txn_oldest_scan(session, &oldest_id, &last_running, &metadata_pinned, &oldest_session);

    /* Update the public IDs. 进行更新操作 */
    ...
    if (WT_TXNID_LT(txn_global->last_running, last_running)) {
        ...
        // 输出日志：输出关于长时间运行事务的详细消息，但仅在取得一些进展时。
...
__txn_config_operation_timeout(WT_SESSION_IMPL *session, const char *cfg[], bool start_timer)
{
     ...
     * 默认配置值为 0，我们无法判断他们是否将其设置回 0，或者是否自动传入默认值。
     */
    if (cval.val != 0) {
         ...
         * optimer 通常会在进入 API 调用时启动，但是当我们在内部配置它时，我们需要单独启动它。
         */
        if (start_timer)
...
__wt_txn_config(WT_SESSION_IMPL *session, const char *cfg[])
{
     ...
     ** 默认同步设置是从 根conn 继承的，但可以被此事务的显式"同步设置"覆盖。
    ...
    if (cval.val == 0 || cval.val == 1)
         ...
         * commit_transaction.
         * 这是同步的显式设置。 设置标志，以便我们知道不要在 commit_transaction 中覆盖它。
         */
...
err:
    if (ret != 0)
         ...
         * 如果我们在配置过程中出错，我们应该清除事务上的标志，这样它们就不会在随后的事务开始调用中设置。
         */
        txn->flags = 0;
    return (ret);
}
 ...
 **     重置txn各种元信息: global_id, id, flags, ts, log等等
 */
void
...
__wt_txn_commit(WT_SESSION_IMPL *session, const char *cfg[])
{
    ...
    if (txn->logrec != NULL && FLD_ISSET(conn->log_flags, WT_CONN_LOG_ENABLED) &&
      !F_ISSET(session, WT_SESSION_NO_LOGGING)) {
        ...
        locked = true;
        WT_ERR(__wt_txn_log_commit(session, cfg));// 入log持久化
    }
...
__wt_txn_rollback(WT_SESSION_IMPL *session, const char *cfg[])
{
     ...
     * page within each file are done at the same time.
     ** 处理已经准备好的更新的成本是比较高的，
     ** 所以对每个文件的每个page内的所有更新并发同时做排序, 可以增加速度
     */
    if (prepare)
        __wt_qsort(txn->mod, txn->mod_count, sizeof(WT_TXN_OP), __txn_mod_compare);
 ...
 **     在 session 创建时初始化好snapshot数组，声明周期随着 session
 */
int
__wt_txn_init(WT_SESSION_IMPL *session, WT_SESSION_IMPL *session_ret)
{
    ...
    // 可以看到 snapshot 大小其实和 session 数量有直接关系
