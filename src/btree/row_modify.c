...
)
{
     ...
     ** 如果是修改：1.当前所有key构建一个修改数组，2.构建点前事务upd，3.将当前upd插入到对应key的mvcc链表上【串行行为】
     *
     ...
     ** 如果是插入：1.构建一个插入数组【每个单元应该是一个跳表】数组长度对应现在page key个数+1
     ** 2.构建一个key结构体和value结构体，3.将key结构体插入到对应的跳表中【串行行为】
     */
    // 找到过旧值，原先存在，对应是修改
    if (cbt->compare == 0) {
        // 当前page还没有被修改或者插入过，此为当前page的第一次修改
        if (cbt->ins == NULL) {
        ...
        // 好像是mvcc增加头结点, 所以是串行化的，涉及判断是否回滚，冲突检测等核心逻辑，如果err直接goto语句
        ...
        // insert原理如同在skip-list中添加节点，所以会有insert-stack这个数据结构，为了多层多次插入
...
err:
        ...
        // 释放此事务生成的所有op内存申请，unmodify
        if (logged)
            __wt_txn_unmodify(session);

        /* Free any allocated insert list object. */
        // 释放插入操作，如果是由本次事务带来的插入？
  ...
  WT_SESSION_IMPL *session, WT_CURSOR_BTREE *cbt, WT_UPDATE *upd, bool update_accounting)
{
     ...
     * 我们不能丢弃这个 WT_UPDATE 结构，我们只能丢弃它后面的 WT_UPDATE 结构，
     * 其他控制线程将终止他们在这个元素中的遍历。 保存对我们将丢弃的列表的引用，并终止该列表。
     */
