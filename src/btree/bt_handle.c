 ...
 * __wt_btree_open --
 *     Open a Btree.
 **     op_cfg有多个配置字符串，有对应ckpt信息，可以拿到root信息从而加载root->page->dsk\
 **     有了bm就可以加载page[root/internal 4k and leaf 32k], memory_page_max=5MB
 */
int
__wt_btree_open(WT_SESSION_IMPL *session, const char *op_cfg[])
{
    ...
    /* Initialize and configure the WT_BTREE structure. 装载配置信息, page大小，等等和btree的所有配置信息 */
    WT_ERR(__btree_conf(session, &ckpt));

    /*
     * Get an opener abstraction that the block manager can use to open any of the files that
     * represent a btree. In the case of a tiered Btree【对应多个文件】, that would allow opening different files
     * according to an object id in a reference. For a non-tiered Btree【对应单个文件】, the opener will know to
     * always open a single file (given by the filename).
     */
    // 如果是单个文件，可以直接拿到对应文件名字
    WT_ERR(__wt_tiered_opener(session, dhandle, &opener, &filename));

    /* Connect to the underlying block manager. 打开block manager，操作磁盘的先决条件*/
    // 初始化bm方法，初始化bm中的核心block，创建文件句柄，校验文件头block
    ...
    if (!F_ISSET(btree, WT_BTREE_SALVAGE | WT_BTREE_UPGRADE | WT_BTREE_VERIFY)) {
         ...
         */  // 将ckpt信息加载到bm中, 方便bm加载tree root page
        ...
        else { // 4k root page, 具体真实加载root page到dsk中
     ...
     */  // 设置evict开关, 只有打开evict的btree才会被evict
    if (btree->original ||
...
/*
 * __btree_conf --
 *     Configure a WT_BTREE structure. 结构体空间之前已经申请，现在是装载配置信息
 */
static int
...
__wt_btree_tree_open(WT_SESSION_IMPL *session, const uint8_t *addr, size_t addr_size)
{
    ...
    // 读取root【猜测是root】 page的数据到dsk缓冲区，并检验
