...
__curtable_insert(WT_CURSOR *cursor)
{
     ...
     * duplicate key.
     ** 如果这个表有对应的索引，根据是否已经存在对应记录判断是否要删除旧的索引。
     ** 如果不是覆盖写，需要检测是否有重复key来判断是否可以插入。
     */
    cp = ctable->cg_cursors;
    primary = *cp++;
 ...
 **     此表的所有索引cursors【从元信息根btree管理处获取的】
 */
static int
 ...
 **     分配一个 table 类型的 cursor：函数指针，cursor配置[file type table]，cursor重复利用[session生命范围内]
 */
int
  ...
  WT_CURSOR **cursorp)
{
    ...
    if (columns == NULL)
        // 从这里根据uri可以获取到table结构体，以及对应的colgroup元信息
    ...
    if (table->is_simple) {
        ...
        // 核心: 打开对应数据源cursor, 此处cgroups[0]->name 就是 colgroup:demo,
        // 其中cgroups[0]->source是file级别 【file:demo.wt】, 这里面就涉及到打开具体的btree文件了
        // 然后给btree分配cursor操作对象
    ...
    *cursor = iface;
    ...
    // 构建一个默认的 cursor，并管理到其 session 中
