 ...
 **     从本地disk加载了btree以后，需要创建cursor才能使用，遍历，插入等等赋能
 */
static int
...
/*
 * __wt_curfile_open --  __wt 外部接口，供外使用
 *     WT_SESSION->open_cursor method for the btree cursor type. || 从disk中load btree root
 **     比较核心的步骤先拿到ckpt，根据ckpt才能打开一个btree
 */
int
  ...
  WT_CURSOR **cursorp)
{
    ...
    else //通过一个ckpt才能从一个file中加载btree的root page
