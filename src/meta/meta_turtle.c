...
__metadata_init(WT_SESSION_IMPL *session)
{
     ...
     ** 我们是单线程的，但无论如何都要获取模式锁：较低级别的代码检查它是否已适当同步。
     */
...
__wt_turtle_init(WT_SESSION_IMPL *session)
{
     ...
     ** 丢弃之前运行留下的任何海龟设置文件。 这对正确性无关紧要，它只是清理随机文件。
     */
     ...
     ** 如果我们发现损坏的龟文件，然后删除它并创建一个新的。 我们可能会在创建海龟文件之后和创建元数据文件之前死掉，
     ** 或者更糟糕的是，元数据文件可能处于某种随机状态。 确保不会发生这种情况：如果我们没有找到龟文件，
     ** 首先创建元数据文件，加载任何热备份，然后创建龟文件。 不管发生什么，如果元数据文件创建没有完全完成，
     ** 我们将不会有一个海龟文件，我们将重复这个过程直到我们成功。
     *
     ...
     ** 增量备份只有在运行恢复并且变为活动状态时才会发生。 所以，如果有一个turtle文件和一个增量备份文件，
     ** 那就是一个错误。 否则，如果已经有一个海龟文件，我们就完成了。
     */
    ...
    if (exist_turtle) {
        ...
        if (ret != 0) {
             ...
             ** 设置一个标志来指定我们应该验证我们是否可以在看到的龟文件版本上启动。 如果我们不能返回错误。
             ** 只检查我们是否没有运行打捞，或者打捞没有读取失败。
             */
         ...
         ** 我们需要检测可能因增量备份文件而崩溃的源数据库和错误运行恢复的目标数据库之间的差异。
         */
         ...
         * from the backup.
         ** 如果我们有一个备份文件和元数据以及海龟文件，我们希望从备份中重新创建元数据。
         */
        if (exist_backup) {
    ...
    if (load) {
        ...
        /* Create the metadata file. 创建元数据文件。 */
        WT_RET(__metadata_init(session));

        /* Load any hot-backup information. 加载任何热备份信息。 */
        WT_RET(__metadata_load_hot_backup(session));

        /* Create any bulk-loaded file stubs. 创建任何批量加载的文件存根。*/
        WT_RET(__metadata_load_bulk(session));
    }

    if (load || load_turtle) {
        /* Create the turtle file. 创建海龟文件。 */
    ...
    /* Remove the backup files, we'll never read them again. 删除备份文件，我们再也不会阅读它们了。 */
