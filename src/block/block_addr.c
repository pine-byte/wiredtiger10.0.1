  ...
  uint32_t *objectidp, wt_off_t *offsetp, uint32_t *sizep, uint32_t *checksump)
{
     ...
     * the trailing尾部 object ID if there are bytes following the triple. The checkpoint cookie is more
     * complicated复杂: it has four address blocks all having the same object ID. Rather than retrofit改装
...
/*
 * __block_ckpt_unpack --
 *     Convert a checkpoint cookie into its components. 从ckpt中解析数据给ci结构体
 */
static int
