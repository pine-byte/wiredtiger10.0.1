  ...
  const char *cfg[], bool forced_salvage, bool readonly, uint32_t allocsize, WT_BLOCK **blockp)
{
    ...
    *blockp = block = NULL;
    ...
    // 在conn中查查看是否已经有打开文件过的first block
    conn = S2C(session);
...
__desc_read(WT_SESSION_IMPL *session, uint32_t allocsize, WT_BLOCK *block)
{
    ...
    /* Use a scratch buffer to get correct alignment for direct I/O. */
    WT_RET(__wt_scr_alloc(session, allocsize, &buf));
    // 感觉像是有个固定的文件头4k,这个里面感觉存储了一些文件的元信息,主要是文件摘要信息,确定文件是否可用
