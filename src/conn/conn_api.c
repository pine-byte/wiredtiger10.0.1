  ...
  WT_CONNECTION **connectionp)
{
    ...
    *connectionp = NULL;
     ...
     ** 立即将结构链接到连接结构列表中：在该列表中查看的唯一内容是数据库名称，NULL 值很好。
     */
     ...
     ** 按以下顺序构建临时的初始配置堆栈（后面的条目会覆盖早期的条目）
     * 1. the base configuration for the wiredtiger_open call
     ** Wiredtiger_open 调用的基本配置
     ...
     ** 在读/写文件系统之前，我们需要知道是否配置为只读或内存行为。
     ** 在我们接触文件系统之前，应用程序可以配置它的唯一方法是wiredtiger 配置字符串或WIREDTIGER_CONFIG 环境变量。
     ...
     * the only way.
     ** 出于安全原因，该环境默认不受信任； 如果应用程序希望我们在读取文件系统之前信任环境，
     ** wiredtiger_open 配置字符串是唯一的方法。
     */
     ...
     ** 配置和分配哈希桶。 这必须在调用加载扩展之前完成。 加密或文件系统等一些扩展可能会分配散列数组。
     */
     ...
     ** 在进行进一步初始化之前加载早期扩展（一个早期扩展是配置文件系统）。
     */
     ...
     ** 如果应用程序没有配置自己的文件系统，请配置我们的一个。 检查以确保我们有一个有效的文件系统。
     */
    if (conn->file_system == NULL) {
     ...
     ** 捕获 config_base 设置文件以供以后使用。 同样，如果应用程序不希望我们读取基本配置文件，则
     ** WIREDTIGER_CONFIG 环境变量或 Wiredtiger_open 配置字符串是唯一的方法。
     */
    ...
    // 如果 config_base_set 为 false，则忽略 base_config 文件。
    if (config_base_set)
     ...
     ** 只读和内存设置可能已在配置文件中设置（不是最佳的，但我们可以处理）。
     ** 再次获取这些设置，以便我们可以在处理其他配置设置时覆盖它们。
     */
    WT_ERR(__wt_config_gets(session, cfg, "in_memory", &cval));
    if (cval.val != 0)
        F_SET(conn, WT_CONN_IN_MEMORY);
    ...
    if (F_ISSET(conn, WT_CONN_READONLY)) {
         ...
         ** 以合并的配置为基础创建一个新堆栈。 只读字符串将使用条目 1，然后我们将再次合并它。
         */
        cfg[0] = merge_cfg;
        cfg[1] = NULL;
        cfg[2] = NULL;
         ...
         ** 我们覆盖了一些只读的配置设置。 其他与只读冲突和错误的设置稍后在其各自的位置进行测试。
         */
     ...
     ** 我们还不能打开会话，所以任何导致会话打开的配置都必须在 __wt_connection_open 中处理。
     *
     ...
     ** 错误消息配置可能已更改（如果设置在配置文件中，而不是在应用程序的配置字符串中），请再次获取。
     ** 先做，纠正错误信息。 同上详细配置，因此我们转储应用程序想要查看的所有内容。
     */
    WT_ERR(__wt_config_gets(session, cfg, "error_prefix", &cval));
    if (cval.len != 0) {
     ...
     ** 如果未配置缓冲区对齐，则使用零，除非还配置了直接 I/O，在这种情况下使用构建时默认值。
     ** 解析写入的代码也在这里，因为它与直接 I/O 几乎相同。
     */
     ...
     ** 如果未设置日志扩展长度，则使用配置的最大日志文件大小的默认值。
     ** 在将其作为日志服务器初始化的一部分进行初始化之前，该大小是未知的。
     */
    ...
    for (ft = file_types; ft->name != NULL; ft++) {
        ...
        if (ret == 0) {
            ...
            case WT_DIRECT_IO_LOG:
                 ...
                 ** 使用“file_extend=(log=)”时，返回的 val 为 1。在这种情况下取消设置日志扩展长度以使用默认值。
                 */
                if (sval.val == 1)
                    conn->log_extend_len = WT_CONFIG_UNSET;
                else if (sval.val == 0 ||
     ...
     ** 打开连接，然后重置本地会话，因为在 __wt_connection_open 中分配了真正的会话。
     */
     ...
     ** 初始化完成后加载扩展； 扩展期望其他一切都到位，并且扩展回调到库中。
     */
     ...
     * for some operations.
     ** 对分层存储做一些早期初始化，因为这可能会影响我们为某些操作选择文件系统。
     */
     ...
     ** 元数据/日志加密器是在扩展之后配置的，因为扩展可能会加载加密器。 我们必须在创建元数据文件之前执行此操作。
     ...
     ** 加密自定义回调需要完全实现的一组加密参数，因为简单地抓取“加密”是行不通的。 例如，
     ** 当前调用的配置可能只是“encryption=(secretkey=xxx)”，其中 encryption.name、
     ** encryption.keyid 从存储的基本配置“继承”。
     */
     ...
     ** 我们需要在这里解析日志配置来验证兼容性设置，因为我们可能需要日志路径和加密和压缩设置。
     */
     ...
     ** 检查海龟和元数据文件，必要时创建它们（避免应用程序线程稍后争相创建元数据文件）。 元数据文件存在后，
     ** 在连接会话中获取对它的引用。
     ...
     ** TURTLE 文件必须是初始化数据库主目录时创建的最后一个文件，这是我们用来决定是否创建的文件。
     */
     ...
     ** 如果用户想要打捞，请在打开元数据光标之前进行。 我们在调用 wt_turtle_init 之后执行此操作，
     ** 因为这会从备份中移动元数据文件，并且会覆盖我们在调用之前所做的任何打捞。
     */
    if (F_ISSET(conn, WT_CONN_SALVAGE)) {
        wt_session = &session->iface;
     ...
     ** 加载任何增量备份信息。 这会读取元数据，因此必须在海龟文件初始化后完成。
     */
     ...
     ** 如果用户想要验证 WiredTiger 元数据，请验证历史存储，因为元数据表可能已被抢救，并且已启动驱逐和恢复运行。
     ...
     ** 哈希数组大小需要很早就设置。 将它们设置在此处的统计信息中。 在早期配置功能中设置它们会使它们归零。
     */
     ...
     ** 默认会话在此之后不应打开数据句柄：因为它可以在线程之间共享，所以依赖 session->dhandle 是不安全的。
     */
    F_SET(session, WT_SESSION_NO_DATA_HANDLES);
...
err:
     ...
     ** 在使用虚拟会话或随后创建的真实会话时，我们可能已经分配了暂存内存，并且我们不想在它们中的任何一个中为其余的运行占用内存。
     */
    ...
    if (ret != 0) {
         ...
         ** 如果我们返回运行恢复错误或恢复未完成，请设置恐慌，这样我们就不会尝试检查点数据句柄。
         ** 我们需要一个显式标志而不是检查 WT_CONN_LOG_RECOVER_DONE 是否未设置，因为在恢复之前的其他错误不会设置该标志。
         */
         ...
         * the connection.
         ** 如果我们检测到数据损坏问题，我们真的想指出损坏而不是设置的任何错误。 我们不能使用标准返回宏，
         ** 因为我们不想一概而论。 在我们有连接时记录在这里，并在我们销毁连接后设置它。
         */
         ...
         ** 根据错误，关闭连接可能会再次返回 WT_PANIC。 因此，如果我们检测到上面的损坏，请在关闭后将其设置在这里。
         */
        if (try_salvage)
            ret = WT_TRY_SALVAGE;
    }
