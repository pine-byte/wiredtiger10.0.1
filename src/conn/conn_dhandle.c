 ...
 **     从conn级别上申请dhandle【结构体空间】，注意 dhandle->type
 */
int
__wt_conn_dhandle_alloc(WT_SESSION_IMPL *session, const char *uri, const char *checkpoint)
{
    ...
    if ((ret = __wt_conn_dhandle_find(session, uri, checkpoint)) != WT_NOTFOUND)
        return (ret);
    // 申请对应的dhandle结构体，dhandle原生对应file
 ...
 **     打开具体的btree handle type,可以打开table类型的,也可以打开file类型的
 */
int
__wt_conn_dhandle_open(WT_SESSION_IMPL *session, const char *cfg[], uint32_t flags)
{
    ...
    __conn_dhandle_config_clear(session);
    WT_ERR(__conn_dhandle_config_set(session));//从meta中获取元信息
    WT_ERR(__conn_dhandle_config_parse(session));//解析元信息，从而为不同type初始化基础信息
    ...
    case WT_DHANDLE_TYPE_BTREE:
        ...
        // 具体打开一个btree from file, dhandle->name存放了对应的key信息, file:demo.wt
        WT_ERR(__wt_btree_open(session, cfg));
        break;
    case WT_DHANDLE_TYPE_TABLE: // 第一次会走到这里来, 逻辑表 table:demo
