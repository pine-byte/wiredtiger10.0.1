...
__wt_logmgr_open(WT_SESSION_IMPL *session)
{
     ...
     */
    conn->log_file_tid.name_init = true;
    conn->log_file_tid.name = "log_file";
    ...
    conn->log_wrlsn_tid.name_init = true;
    conn->log_wrlsn_tid.name = "log_wrlsn";
    WT_RET(__wt_thread_create(
    ...
    if (conn->log_session != NULL) {
         ...
         * Start the thread.
         */
        conn->log_tid.name_init = true;
        conn->log_tid.name = "log";
        WT_RET(
