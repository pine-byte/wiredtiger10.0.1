 ...
 * object.
 ** WiredTiger使用代来管理各种资源。线程在访问资源之前发布当前生成，并在完成后清除。
 ** 例如，一个想要替换内存中对象的线程会替换该对象并增加对象的生成。
 ** 一旦没有线程发布上一代，就可以安全地放弃对象的上一版本。
 */
 ...
 **     发布线程的资源生成。
 **     比较 [session generation] vs [conn generation]
 */
void
__wt_session_gen_enter(WT_SESSION_IMPL *session, int which)
{
     ...
     ** 不要进入我们已经在其中的一代，它可能会导致代码受到在一代之外运行的一代的保护。
     */
     ...
     * generation.
     ** 分配线程的资源生成并发布它，确保等待资源耗尽的线程看到新值。
     ** 检查我们在发布后没有与世代更新竞争，我们依赖于在扫描最老一代时不会错过发布的值。
     */
    do {
