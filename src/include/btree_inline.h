...
#define WT_KEY_FLAG_BITS 0x03
    //CELL_FLAG：0x01 表示这个row标示了key的cell_t对象在dsk内存缓冲区的位置，因为key和value的值有可能很大，
    //一个page存储不下，这个时候cell_t内的data可能只是ovfl addr。由于单个key/value对是连续存放对，
    //因此能够通过key cell_t找到相应的value cell_t。
...
// K_FLAG：0x02，表示这个row标示了k/v对中key data的位置，因为value的CELL是紧跟在key存储的位置后面，
// 找到key就能读取value的cell并找到value。
...
// KV_FLAG：0x03 表示这个row同时标示了k/v中key data和value data 的位置。
...
#define WT_KV_ENCODE_KEY_CELL_OFFSET(v) ((uintptr_t)(v) << 2)
    ...
    case WT_CELL_FLAG: /* On-page cell. */
        ...
        break;
    case WT_K_FLAG: /* Encoded key. 主要用于internal节点 */
        if (ikeyp != NULL)
            *ikeyp = NULL;
        if (cellp != NULL)
        ...
        break;
    case WT_KV_FLAG: /* Encoded key/value pair. 主要用于leaf节点 */
        if (ikeyp != NULL)
            *ikeyp = NULL;
        if (cellp != NULL)
 ...
 *     取出来的是key值【对应的类型，本人测试过程中是字符串类型】给到 *key
 */
static inline int
__wt_row_leaf_key(
