...
struct __wt_connection_impl {
    ...
    const char *home;         /* Database home 数据目录地址 */
...
#define WT_CONN_COMPAT_NONE UINT16_MAX
     ...
     * of the table URI.
     ** 该连接保留数据句柄的缓存。 句柄集可以变得非常大，因此我们同时维护一个简单列表和一个列表哈希表。
     ** 散列表键基于表 URI 的散列。能够通过表URI快速定位表的句柄。
     */
    ...
    TAILQ_HEAD(__wt_blockhash, __wt_block) * blockhash; //这个事按照 map<文件名,block> hashcache
     ...
     **  WiredTiger 默认为 50 个并发会话（控制线程）分配空间。
     **  动态增加线程的数量是可能的，但很棘手，因为服务器线程正在遍历数组而不锁定它。
     ...
     **  有一个 WT_SESSION_IMPL 指针数组引用分配的数组；
     **  我们这样做是因为我们想要一种简单的方法让服务器线程代码避免在只有几个线程运行时遍历整个数组。
     */
    WT_SESSION_IMPL *sessions; /* Session reference */
    uint32_t session_size;     /* Session array size */
    uint32_t session_cnt;      /* Session count */

    size_t session_scratch_max; /* Max scratch memory per session  每个会话的最大暂存内存 */

    WT_CACHE *cache;              /* Page cache 可能是LRU算法设计 */
    volatile uint64_t cache_size; /* Cache size (either statically
                                     configured or the current size
                                     within a cache pool). 静态配置或缓存池中的当前大小 */

    // 全局事务分配管理器，全局只有一个，只有conn才有。且每个sesssion只有一个对应的tnx，全局共有多个session
    // 多个session进行并发竞争
    WT_TXN_GLOBAL txn_global; /* Global transaction state */

    /* Recovery checkpoint snapshot details saved in the metadata file */
    // 元数据文件中保存的恢复检查点快照详细信息
...
#define WT_CKPT_LOGSIZE(conn) ((conn)->ckpt_logsize != 0)
    ...
    /* Checkpoint stats and verbosity timers 检查点统计信息和详细计时器 */
    ...
    /* Checkpoint progress message data 检查点进度消息数据 */
    uint64_t ckpt_progress_msg_count;
    uint64_t ckpt_write_bytes;
    uint64_t ckpt_write_pages;

    /* Checkpoint and incremental backup data 检查点和增量备份数据 */
...
#define WT_CONN_LOG_ZERO_FILL 0x800u       /* Manually zero files */
    ...
    WT_LOG *log;                           /* Logging structure wal入口 */
...
#define WT_CONN_DEBUG_UPDATE_RESTORE_EVICT 0x20u
    ...
    /* Verbose settings for our various categories. 我们各种类别的详细设置。感觉像是日志 */
