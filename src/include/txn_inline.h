 ...
 **     只有落实 op 时，才给分配具体的 txn id，所有此txn 的 ops 共享一个事务id
 **     落实当前 op 给当前 txn
 */
static inline int
...
/*
 * __wt_txn_modify -- 修改增加upd，upd中有新value, 附件事务选项
 *     Mark a WT_UPDATE object modified by the current transaction.
 */
static inline int
...
/*
 * __wt_txn_upd_visible -- 感觉像是看看能不能正常访问，如果不能就回滚【是否影响正常访问】
 *     Can the current transaction see the given update.
 *     【以snapshot多版本ts为依据去做尝试遍历】
 */
static inline bool
...
__wt_txn_begin(WT_SESSION_IMPL *session, const char *cfg[])
{
     ...
     ** 如果需要分配快照 或者 更新现有快照。
     ** 不要更新自动提交事务的现有快照，因为它们是在操作结束时提交的。【隐式事务】
     */
    if (txn->isolation == WT_ISO_SNAPSHOT &&
      !(F_ISSET(txn, WT_TXN_AUTOCOMMIT) && F_ISSET(txn, WT_TXN_HAS_SNAPSHOT))) {
         ...
         ** 如果缓存已满，请在此处停止。 我们已经分配了一个事务 ID，这使得驱逐可以决定我们是造成问题的原因
         ** 并返回 WT_ROLLBACK。 WT_SESSION.begin_transaction API 无法返回回滚，继续。
         */
        WT_RET_ERROR_OK(__wt_cache_eviction_check(session, false, true, NULL), WT_ROLLBACK);
        // 在开启事务的时候，就获取并锁定了快照, 比较核心
 ...
 **     申请分配一个txn id给当前session txn中的所有op
 */
static inline uint64_t
  ...
  WT_SESSION_IMPL *session, WT_CURSOR_BTREE *cbt, WT_UPDATE *upd, wt_timestamp_t *prev_tsp)
{
    ...
    // 看看upd能不能正常访问，一直到可见的upd链表中的节点位置，先把当前已经aborted的节点都干掉
