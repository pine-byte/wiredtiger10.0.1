 ...
 **    内部核心创建一个 cursor，主要是区分创建什么类型的 cursor
 **     int 我理解是按照字符串第一个字符按switch int映射类型
 */
static int
  ...
  WT_CURSOR *other, const char *cfg[], uint64_t hash_value, WT_CURSOR **cursorp)
{
    ...
    *cursorp = NULL;
     ...
     */
    case 'f':
        if (WT_PREFIX_MATCH(uri, "file:")) // 打开具体的wt文件,bm,cursor等等
            WT_RET(__wt_curfile_open(session, uri, owner, cfg, cursorp));
        break;
    case 'm':
 ...
 **     根据file uri 打开btree，并分配对应的cursor
 */
int
  ...
  WT_CURSOR **cursorp)
{
    ...
    // 递归共打开了两次：第一次table:demo, 第二次file:demo【有些像状态机的感觉】
