 ...
 *     set session->dhandle. 通过ckpt打开file加载到btree root page
 */
int
__wt_session_get_btree_ckpt(
...
retry:
    ...
    // 通过uri file:demo 加载具体的 ckpt信息 ，然后在从disk加载 btree root page
  ...
  const char *cfg[], uint32_t flags)
{
    ...
    for (;;) {
        // 搜索不成就创建【from conn】, 主要是申请空间结构体, dhandle handle btree
         ...
         * enforce this.
         ** 独占锁, 只打开一次，从一个table 逻辑meta root开始, 最多递归2次而已
         */
        ...
        /* Open the handle. */
        // table 类型: 从 table:demo 到 colgroup:demo
        // file  类型: 直接打开file:demo.wt
