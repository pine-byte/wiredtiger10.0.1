 ...
 *     Open the column groups for a table. 第一次从 table:demo 到 colgroup:demo
 **     打开table的column groups【无论是否simple】, 利用table信息组建group信息
 */
int
__wt_schema_open_colgroups(WT_SESSION_IMPL *session, WT_TABLE *table)
{
    ...
    /* Open each column group. 简单表的话就为一个 */
 ...
 **     从元信息（所有表，索引记录的地方）获取对应的索引cursor
 */
static int
__schema_open_index(
 ...
 **     核心：此处为打开一个table类型的实现，对应逻辑表 cursor 为 WT_TABLE
 **     一个逻辑table可能带出一组cursor【source，index等等】此处多为元配置信息
 */
static int
__schema_open_table(WT_SESSION_IMPL *session)
{
    ...
    // 申请空间，并初始化colgroups, 这里会用到 table:demo 获取到 colgroup:demo
    // 核心函数，前面都是在解析colgroup信息, 此处并未打开第一文件
 ...
 *     Open a named table. 从 table:demo 到 colgroup:demo
 */
int
